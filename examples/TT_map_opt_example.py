...
#bank = strain_ds[:, :len_freq].astype(np.complex128)

for gwf in range(num_data):
    job_id_read_save = job_id + gwf
    ...
    num_bins = 16
    SNR_CUT    = 5.5                       # SNR cut for hopless detection
    dof      = (num_bins * 2) - 2
    ...
    for i in tqdm(range(num_templates)):
        ...
        #hp = pycbc.types.frequencyseries.FrequencySeries(bank[i], delta_f=1.0/80, copy=False)
        ...
        # chi-sq ONLY at the surving SNR values
