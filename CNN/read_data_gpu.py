...
def create_tf(input_path, img_width, img_height, batch_size, cache_dir=None):
    ...
    # Configure the data-reading performance: decode the PNGs once (cache to a
    # file path, not memory), reshuffle every epoch, then batch/prefetch.
    def configure_for_performance(ds, cache_path):
    ...
    train_ds = configure_for_performance(train_ds, train_cache)
    val_ds = configure_for_performance(val_ds, val_cache)
    ...
    # The test set must stay ordered: probs[i] is paired with file_path_test[i],
    # so no shuffling and deterministic parallel map/batch only, no cache needed.
    test_ds = list_test_ds.map(process_path_test, num_parallel_calls=AUTOTUNE, deterministic=True)
