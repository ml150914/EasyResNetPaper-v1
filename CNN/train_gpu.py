...
from read_data_gpu import create_tf
...
def main():
    ...
    train_ds, val_ds, test_ds, file_path_test = create_tf(
        ...
        cache_dir=os.path.join(args.data_out, "tfcache"))
    ...
    callbacks = [
        ...
        keras.callbacks.ModelCheckpoint(
            os.path.join(args.data_out, f"{args.model}_ckpt.keras"),
            monitor="val_loss", save_best_only=True, verbose=1),
        keras.callbacks.CSVLogger(
            os.path.join(args.data_out, "training_history.csv"), append=True),
    ]
