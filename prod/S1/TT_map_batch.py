...
for gwf in range(num_data):
    ...
    #strain_ds = bank_file['strain']
    #strain_ds = all_templates['strain']
    row_buf_c128 = np.empty(len_freq, dtype=np.complex128)
    row_buf_f32 = np.empty(len_freq, dtype = np.float32)
    for i in tqdm(range(num_templates)):
        # Read the template
        #hp_np = all_templates[i,:len_freq]
        ...
        #np.copyto(row_buf_f32, row_buf_c128.real, casting='unsafe')
        hp = pycbc.types.frequencyseries.FrequencySeries(
                row_buf_c128,       # view into the buffer — no extra copy
                delta_f = 1/80,
                copy=False,    # tell PyCBC not to copy the array again
            )
        #hp = pycbc.types.frequencyseries.FrequencySeries(all_templates[i,:len_freq], delta_f = 1/80)
        
        # Calculate the complex (two-phase) SNR
        snr_ts  = matched_filter(hp, data, psd=psd,
        ...
        snrs[row, :]    = abs(snr_ts.crop(59, 17)).numpy()
        del snr_ts
        ...
        chisq_ts = pycbc.vetoes.power_chisq(hp, data, num_bins, psd = psd, low_frequency_cutoff=args.low_frequency_cutoff_matched_filtering, high_frequency_cutoff=args.high_frequency_cutoff_matched_filtering)
            
        chisq_ts /= (num_bins * 2) - 2
        #chisq = chisq.crop(59, 17)
        chisqs[row, :]  = chisq_ts.crop(59, 17).numpy()
        del chisq_ts, hp
    ...
    rwsnrs = np.where(chisqs>1, snrs / ((1 + chisqs**3) / 2) ** (1/6), snrs);
